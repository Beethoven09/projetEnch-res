...
public class EnchereNonConnecte {
	...
	public List<ArticleVendu> listerEncheresEnCours(String nomArticle, String categorie) {
		...
		if (nomArticle != null && !nomArticle.isEmpty()) {
			articles = articles.stream().filter(a -> a.getNom().toLowerCase().contains(nomArticle.toLowerCase()))
					.collect(Collectors.toList());
		}

		// Filtrage par catégorie
		if (categorie != null && !categorie.isEmpty()) {
			articles = articles.stream().filter(a -> a.getCategorie().toLowerCase().equals(categorie.toLowerCase()))
					.collect(Collectors.toList());
		}
